...
{
    if (available < WSFilteredBlockBaseLength) {
        WSErrorSetNotEnoughBytes(error, [self class], available, WSFilteredBlockBaseLength);
        return nil;
    }
    ...
    offset += WSBlockHeaderLength - sizeof(uint8_t);
